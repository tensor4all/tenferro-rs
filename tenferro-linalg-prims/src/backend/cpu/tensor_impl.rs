...
use super::super::col_major_strides;
use super::super::tensor_api::{
...
use super::super::tensor_helpers::{
...
fn first_failing_leading_principal_minor<T: KernelLinalgScalar>(
    ...
    for k in 1..=n {
        ...
        if super::cholesky_slices(&minor_a[..minor_len], k, &mut minor_l[..minor_len]).is_err() {
            return Ok(k as i32);
        }
    }
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        ...
        l_buf.fill(T::zero());
        u_buf.fill(T::zero());
        super::lu_slices(a_slice, n, n, &mut perm, &mut l_buf, &mut u_buf)?;

        let batch_info = first_zero_pivot_from_u(&u_buf, n, n);
        info[i] = batch_info;
        if batch_info == 0 {
            let solution_slice = &mut solution_data[i * mat_b..(i + 1) * mat_b];
            super::solve_slices(a_slice, b_slice, n, nrhs, solution_slice)?;
        }
    }
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        ...
        super::solve_slices(a_slice, b_slice, n, nrhs, x_slice)?;
    }
    ...
    T: KernelLinalgScalar,
{
    ...
    for batch in 0..bc {
        ...
        super::solve_triangular_slices(&lower, &permuted_rhs, n, rhs.nrhs, false, &mut tmp)?;
        super::solve_triangular_slices(
            &upper,
            &tmp,
            n,
            rhs.nrhs,
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        ...
        let b_slice = &b_data[b_off + i * mat_b..b_off + (i + 1) * mat_b];
        let x_slice = &mut x_data[i * mat_b..(i + 1) * mat_b];
        super::solve_triangular_slices(a_slice, b_slice, n, nrhs, upper, x_slice)?;
    }
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        ...
        q_buf.fill(T::zero());
        r_buf.fill(T::zero());
        super::qr_slices(a_slice, m, n, &mut q_buf, &mut r_buf)?;
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        let a_slice = &a_data[a_off + i * mat_size..a_off + (i + 1) * mat_size];
        super::thin_svd_slices(a_slice, m, n, &mut u_buf, &mut s_buf, &mut vt_buf)?;
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        ...
        l_buf.fill(T::zero());
        u_buf.fill(T::zero());
        super::lu_slices(a_slice, m, n, &mut perm, &mut l_buf, &mut u_buf)?;
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        let a_slice = &a_data[a_off + i * mat_size..a_off + (i + 1) * mat_size];
        l_buf.fill(T::zero());
        super::cholesky_slices(a_slice, n, &mut l_buf)?;
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        ...
        match super::cholesky_slices(a_slice, n, &mut l_buf) {
            Ok(()) => {
                l_data[i * mat_size..(i + 1) * mat_size].copy_from_slice(&l_buf);
            }
            Err(_) => {
    ...
    T: KernelLinalgScalar,
{
    ...
    for i in 0..bc {
        let a_slice = &a_data[a_off + i * mat_size..a_off + (i + 1) * mat_size];
        super::eigen_sym_slices(a_slice, n, &mut val_buf, &mut vec_buf)?;
    ...
    T: KernelLinalgScalar,
{
    ...
    let (val_ri_len, vec_ri_len) = super::eig_buffer_sizes::<T>(n);
    ...
    for i in 0..bc {
        let a_slice = &a_data[a_off + i * mat_size..a_off + (i + 1) * mat_size];
        super::eig_slices(a_slice, n, &mut val_ri, &mut vec_ri)?;

        super::eig_ri_to_complex::<T>(
            n,
            &val_ri,
            &vec_ri,
