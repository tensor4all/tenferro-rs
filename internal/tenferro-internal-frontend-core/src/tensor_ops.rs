...
pub fn unflatten_index_column_major(mut flat: usize, dims: &[usize], out: &mut [usize]) {
...
pub fn tensor_map_binary_typed<T>(
    lhs: &Tensor<T>,
    rhs: &Tensor<T>,
...
pub fn tensor_map_unary_typed<T, U>(
    input: &Tensor<T>,
...
pub fn tensor_max_typed<T>(input: &Tensor<T>) -> Result<T>
where
...
pub fn tensor_max_abs_diff_typed<T>(lhs: &Tensor<T>, rhs: &Tensor<T>) -> Result<f64>
where
