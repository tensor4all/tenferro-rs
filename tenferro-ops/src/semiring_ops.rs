...
pub trait SemiringOps: GraphOp {
    ...
    /// that carry rank on the op variant so downstream passes do not have to
    /// recover it from the soon-to-be-removed `DotGeneralConfig` rank fields
    /// (issue #664). Implementors that do not track rank (e.g. `SemiringOp`) may
    /// ignore them.
