...
use tenferro_prims::{CpuBackend, CpuContext, CudaBackend, CudaContext};
...
fn public_structured_ops_cover_cpu_and_runtime_error_paths() {
...

    {
        ...
        let out =
            einsum_with_subscripts_in_ctx::<CpuBackend, _, f64>(&mut ctx, &subs, &[&diag, &diag])
                .unwrap();
        assert!(out.is_diag());
        assert_eq!(as_slice(out.payload()), &[4.0, 9.0]);

        let err =
            einsum_with_subscripts_in_ctx::<CpuBackend, _, f64>(&mut ctx, &subs, &[]).unwrap_err();
        assert!(matches!(err, Error::InvalidAdTensor { .. }));
    }

    {
        let mut ctx = CudaContext::new();
        let err = to_dense_in_ctx::<CudaBackend, _, f64>(&mut ctx, &diag).unwrap_err();
        match err {
            Error::UnsupportedRuntimeOp { runtime, .. } => assert_eq!(runtime, "cuda"),
            Error::Backend(_) => {}
            other => panic!("expected UnsupportedRuntimeOp for cuda to_dense, got {other:?}"),
        }

        let err =
            einsum_with_subscripts_in_ctx::<CudaBackend, _, f64>(&mut ctx, &subs, &[&diag, &diag])
                .unwrap_err();
        match err {
            Error::UnsupportedRuntimeOp { runtime, .. } => assert_eq!(runtime, "cuda"),
            Error::Backend(_) => {}
            other => panic!("expected UnsupportedRuntimeOp for cuda einsum, got {other:?}"),
        }
    }
}
