...
fn tensor_shape_methods_cover_non_f64_runtime_variants() {
    ...
    assert_eq!(f32_reshaped.scalar_type(), tenferro::ScalarType::F32);
    ...
    assert_eq!(c32_prefix.scalar_type(), tenferro::ScalarType::C32);
    assert_eq!(c32_prefix.dims(), &[2, 1]);
    let c32_contiguous = c32_matrix.contiguous(MemoryOrder::RowMajor).unwrap();
    assert_eq!(c32_contiguous.scalar_type(), tenferro::ScalarType::C32);
    ...
    .diag_embed(2)
    .unwrap();
    assert_eq!(c64_diag.scalar_type(), tenferro::ScalarType::C64);
