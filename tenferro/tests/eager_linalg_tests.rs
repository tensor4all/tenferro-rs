...
fn eager_linalg_svd_reconstructs_input() {
    let a = EagerTensor::from_tensor_in(
        Tensor::from_vec_col_major(vec![2, 2], vec![3.0_f64, 0.5, 1.0, 2.0]),
        test_ctx(),
    );
...
fn eager_linalg_qr_reconstructs_input() {
    let a = EagerTensor::from_tensor_in(
        Tensor::from_vec_col_major(vec![3, 2], vec![3.0_f64, 1.0, 2.0, 2.0, 0.0, 1.0]),
        test_ctx(),
    );
    let (q, r) = a.qr().unwrap();
...
fn eager_linalg_lu_backward_is_finite() {
    let a = EagerTensor::requires_grad_in(
        Tensor::from_vec_col_major(vec![2, 2], vec![4.0_f64, 1.0, 1.0, 3.0]),
        test_ctx(),
    );
...
fn eager_elementwise_forward_surface_smoke() {
    ...
    let pow_base = EagerTensor::from_tensor_in(
        Tensor::from_vec_col_major(vec![3], vec![8.0_f64, 9.0, 4.0]),
        test_ctx(),
    );
    let exponents = EagerTensor::from_tensor_in(
        Tensor::from_vec_col_major(vec![3], vec![3.0_f64, 0.5, 2.0]),
        test_ctx(),
    );
    assert_close_slice(
...
fn eager_elementwise_scatter_forward_updates_operand() {
    let operand = EagerTensor::from_tensor_in(
        Tensor::from_vec_col_major(vec![4], vec![0.0_f64, 0.0, 0.0, 0.0]),
        test_ctx(),
    );
    let indices = EagerTensor::from_tensor_in(
        Tensor::from_vec_col_major(vec![2, 1], vec![1_i64, 3]),
        test_ctx(),
    );
    let updates = EagerTensor::from_tensor_in(
        Tensor::from_vec_col_major(vec![2], vec![5.0_f64, 4.0]),
        test_ctx(),
    );
    let result = operand
        .scatter(
