...
fn unary_and_reduction_entrypoints_route_through_runtime_dispatch() {
    let scalar_builders = [
        repo_file("src/ops/scalar/primal.rs"),
        repo_file("src/ops/scalar/ad/common.rs"),
        repo_file("src/ops/scalar/ad/unary.rs"),
        repo_file("src/ops/scalar/ad/binary.rs"),
        repo_file("src/ops/scalar/ad/reduction.rs"),
    ]
    .join("\n");
    ...
    let ad_builders = [
        repo_file("src/ops/einsum/ad.rs"),
        repo_file("src/ops/reduction/ad.rs"),
        repo_file("src/ops/linalg/ad/common.rs"),
        repo_file("src/ops/linalg/ad/single.rs"),
        repo_file("src/ops/linalg/ad/slogdet.rs"),
        repo_file("src/ops/linalg/ad/svd_qr.rs"),
        repo_file("src/ops/linalg/ad/lu_lstsq.rs"),
        repo_file("src/ops/linalg/ad/spectral.rs"),
    ]
    .join("\n");
...
fn runtime_helpers_stay_capability_driven() {
    ...
    let runtime_dispatch = repo_file("src/runtime/dispatch.rs");
    let contracts = repo_file("src/runtime/contracts.rs");
    let reduction_builders = repo_file("src/ops/reduction/ad.rs");
