    ...
    let instructions = prog
        ...
        .map(|instr| {
            let op = match &instr.op {
                ...
                StdTensorOp::Convert { to, .. } => StableHloOp::Convert { to: *to },
                ...
                StdTensorOp::Tril { k } => StableHloOp::Tril { k: *k },
                StdTensorOp::Triu { k } => StableHloOp::Triu { k: *k },
    ...
    let instructions: Vec<ExecInstruction> = program
        ...
        .map(|(idx, instr)| {
            let op = match &instr.op {
                ...
                StableHloOp::Convert { to } => ExecOp::Convert { to: *to },
                ...
                StableHloOp::Tril { k } => ExecOp::Tril { k: *k },
                StableHloOp::Triu { k } => ExecOp::Triu { k: *k },
