...
fn reshape_ad_tensor_typed<T>(input: &AdTensor<T>, new_dims: &[usize]) -> Result<AdTensor<T>>
where
...
fn take_prefix_ad_tensor_typed<T>(
    input: &AdTensor<T>,
    axis: usize,
    len: usize,
) -> Result<AdTensor<T>>
...
fn diag_embed_ad_tensor_typed<T>(input: &AdTensor<T>, logical_rank: usize) -> Result<AdTensor<T>>
where
    T: Scalar + Copy + 'static,
{
...
fn contiguous_ad_tensor_typed<T>(input: &AdTensor<T>, order: MemoryOrder) -> Result<AdTensor<T>>
where
    T: Scalar,
{
