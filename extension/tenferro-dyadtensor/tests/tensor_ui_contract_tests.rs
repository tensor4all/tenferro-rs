...
use tenferro_dyadtensor::{
    backward, forward_ad, set_default_runtime, AllowedPairs, BackwardOptions, FactorizeOptions,
    FactorizeResult, Result, RuntimeContext, ScalarType, StructuredTensor, SvdResult, Tensor,
    TensorKernel,
};
...
impl TensorKernel for DummyKernel {
    ...
    fn inner_product(&self, _other: &Self) -> Result<Tensor> {
        Ok(Tensor::from_tensor(scalar_f64(0.0)))
    }
}

#[test]
fn tensor_kernel_uses_rank0_tensor_scalars() {
    let x = DummyKernel;
    let scale = Tensor::from_tensor(scalar_f64(2.0));
    let _ = x.scale(scale).unwrap();
}
