...
fn diag_f32(values: &[f32]) -> Tensor {
    Tensor::from_structured(StructuredTensor::from_diagonal_vector(vector_f32(values), 2).unwrap())
}

fn diag_c64(values: &[Complex64]) -> Tensor {
    Tensor::from_structured(StructuredTensor::from_diagonal_vector(vector_c64(values), 2).unwrap())
}
...
#[test]
fn dynadtensor_public_reverse_api_tracks_requested_gradients() {
