...
| #1365 | Implemented locally; focused A100 verification passed | CUDA fusion classification now returns `Ok(None)` for identity-view inputs with incompatible runtime shapes, allowing the existing unfused fallback. The direct regression first failed with `ShapeMismatch { op: "fused_elementwise", lhs: [3], rhs: [] }`, then passed for `[3]` and `[]`; a companion regression confirms a plan/runtime dtype descriptor mismatch remains a typed `BackendFailure`. | Direct fusion shape-defusal and dtype-corruption regressions pass on an NVIDIA A100; CUDA feature compilation and relevant launch contracts pass. Overall batch verification remains incomplete. | Malformed plans and corrupted tensor descriptors must remain hard errors; only valid-but-unsupported cross-input shape combinations defuse. |
