...
- Default cap: 1 GiB. There is no single optimal cap, so the knob is what
  matters; the default is a finite policy value chosen so that a large workload
  is not silently degraded by the default, and it is documented as neither a
  practical memory protection nor a reservation. `u64::MAX` (effectively
  unlimited by default) was rejected: it leaves no practical protection in
  place for a caller that never configures the cap.
...
- New regression test `cuda_cutensor_shared_workspace_does_not_evict_plan_cache_by_bytes`
  ...
  with the CPU backend after the clear.
- New cap tests, all executed on an A100 80GB PCIe (driver 580.126.09,
  ...
  retained entries and bytes, plan reuse and numerical agreement intact),
...
- Device-free host tests cover the capacity rounding, the retention decision
  ...
  and `u64::MAX`-adjacent arithmetic), and the 1 GiB default. The public
...
- Barrier-fallback counts can be observed with
  ...
  an acceptance criterion and no permanent counter was added.
- Not measured: this change is not claimed as an end-to-end speedup. The
  ...
  1 GiB default is not derived from a measured workspace distribution.
