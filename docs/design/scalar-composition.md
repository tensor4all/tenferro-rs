...
| The adjoint and tangent of a pattern wider than pairwise | refused, deliberately | `ext/df64-proof/tests/einsum_ad.rs` asserts the refusal. Generalising the two helpers is a multi-site change to their payload, accessors, input and output counts, the rule arms, and the fold the body shares, for a niche combination — an N-ary contraction differentiated in Df64 — while the pairwise path it would refactor is the one whose duality the tests check exactly. The iteration policy asks for the next low-risk step, so the refusal is documented instead of the refactor started. |
| bf16 through the ordinary einsum surface | not implemented, with the reason measured | #1793's precision table lists a bf16 CPU einsum with documented f32 accumulation. The bf16 contribution has the scalar, the shared elementwise path, and the reduction contract, but not the extension-module apparatus an operation needs: a module, an engine, a planning config, a prepared operation, and its binding and specialization. `ext/sparse/src/extension.rs` is 1202 lines of exactly that, and `ext/df64-proof/src/extension.rs` is 2454, so the einsum op this branch added could reuse an owning crate's module while a new one cannot. Adding bf16 einsum therefore means either duplicating that apparatus or giving the runtime a shared module facility for reference bodies, and the second is a core API change. |
