...
| The tangent of a pattern wider than pairwise | refused, deliberately | `ext/df64-proof/tests/einsum_ad.rs` asserts the refusal. The adjoint generalised first because its rule already carried the whole operand list, while the tangent's payload records a pair of availability flags; generalising that payload to a mask over N operands is the next piece rather than a piece of the adjoint's | the helper becomes a masked N-operand operation |
