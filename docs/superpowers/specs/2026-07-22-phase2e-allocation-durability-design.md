...
are required. Persisted allocation probe manifests require
`type(protocol_version) is int` as well as the exact supported value, so an
equal float or boolean cannot pass identity validation.
