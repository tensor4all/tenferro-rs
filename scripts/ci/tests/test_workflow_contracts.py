...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_the_paid_workflow_decides_everything_in_one_step(self) -> None:
        """No workflow_call input decides the paid path.

        Measured: job-level `if`s in the called workflow never saw the caller's
        inputs (boolean and textual comparisons), and a skipped caller still
        scheduled these jobs. The decision therefore lives in one step that reads
        the PR's state, its labels, and the change classification.
        """

        execute = read(".github/workflows/runpod-gpu-execute.yml")
        self.assertIn("id: paid_path", execute)
        self.assertIn('echo "run_paid_path=false" >> "${GITHUB_OUTPUT}"', execute)
        self.assertIn("python3 scripts/ci/change_policy.py", execute)
        self.assertIn('paid_path_skipped: ${{ steps.paid_path.outputs.run_paid_path == \'false\' }}', execute)
        self.assertIn(
            "if: steps.paid_path.outputs.run_paid_path != 'false'", execute
        )
        self.assertIn(
            "if: needs.start-runpod.outputs.paid_path_skipped != 'true'", execute
        )
        # The ineffective plumbing is gone: no job-level input conditions, no
        # caller-side forwarding of them.
        self.assertNotIn("inputs.gpu_required", execute)
        self.assertNotIn("inputs.local_gpu_validation", execute)
        text = read(".github/workflows/runpod-gpu-test.yml")
        self.assertNotIn("gpu_required: ${{ needs.authorize.outputs.gpu_required }}", text)
        self.assertNotIn("local_gpu_validation: ${{ needs.authorize.outputs", text)
