...
/// Marker trait for element types accepted by CUDA strided-copy transforms.
///
/// # Examples
///
/// ```rust
/// use tenferro_device::cuda::runtime::StridedCopyTransformElement;
///
/// fn accepts_transform_element<T: StridedCopyTransformElement>() {}
/// accepts_transform_element::<f64>();
/// accepts_transform_element::<num_complex::Complex64>();
/// ```
pub trait StridedCopyTransformElement: Copy + 'static {
    const SUPPORTS_CONJ_STRIDED_COPY: bool;
}

macro_rules! impl_strided_copy_transform_element {
    ($ty:ty, $supports_conj:expr) => {
        impl StridedCopyTransformElement for $ty {
            const SUPPORTS_CONJ_STRIDED_COPY: bool = $supports_conj;
        }
    };
}

impl_strided_copy_transform_element!(f32, false);
impl_strided_copy_transform_element!(f64, false);
impl_strided_copy_transform_element!(i32, false);
impl_strided_copy_transform_element!(i64, false);
impl_strided_copy_transform_element!(u8, false);
impl_strided_copy_transform_element!(bool, false);
impl_strided_copy_transform_element!(Complex32, true);
impl_strided_copy_transform_element!(Complex64, true);
