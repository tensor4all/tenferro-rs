...
fn cpu_arange_and_linspace_construct_expected_sequences() {
    let arange = Tensor::<f64>::arange(0.0, 5.0, 1.0, CPU, MemoryOrder::ColumnMajor);
    assert_dense_f64_sequence(&arange, &[5], CPU, &[0.0, 1.0, 2.0, 3.0, 4.0]);

    let linspace = Tensor::<f64>::linspace(0.0, 1.0, 5, CPU, MemoryOrder::ColumnMajor);
...
fn cuda_arange_and_linspace_construct_expected_sequences() {
    let arange = Tensor::<f64>::arange(0.0, 5.0, 1.0, GPU0, MemoryOrder::ColumnMajor);
    assert_dense_f64_sequence(&arange, &[5], GPU0, &[0.0, 1.0, 2.0, 3.0, 4.0]);

    let linspace = Tensor::<f64>::linspace(0.0, 1.0, 5, GPU0, MemoryOrder::ColumnMajor);
