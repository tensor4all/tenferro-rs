...
#[allow(dead_code)]
pub fn decode_f64_tensor(encoded: &DbTensor) -> Result<Tensor<f64>, String> {
    decode_f64_tensor_with_core_rank(encoded, 0)
}

#[allow(dead_code)]
pub fn decode_f64_tensor_map(
    encoded: &BTreeMap<String, DbTensor>,
) -> Result<BTreeMap<String, Tensor<f64>>, String> {
    encoded
        .iter()
        .map(|(name, tensor)| Ok((name.clone(), decode_f64_tensor(tensor)?)))
        .collect()
}
...
#[allow(dead_code)]
pub fn elementwise_abs(tensor: &Tensor<f64>) -> Tensor<f64> {
    let data: Vec<f64> = tensor_data_col_major(tensor)
        .into_iter()
        .map(f64::abs)
        .collect();
    tensor_from_col_major(data, tensor.dims())
}
