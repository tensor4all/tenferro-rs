...
/// Execute a read-into elementwise operation through a backend's allocating operations.
///
/// Backend implementations use this helper when their device-native operation
/// APIs already provide the correct placement and copy semantics. It deliberately
/// does not inspect or transfer storage across devices.
///
/// # Errors
///
/// Returns [`Error::Validation`] for wrong arity or overlapping output storage,
/// and propagates the selected backend operation and copy errors.
#[doc(hidden)]
pub fn elementwise_read_into_via_allocating_ops<B: TensorElementwise + ?Sized>(
    backend: &mut B,
    ...
    let result = match op {
        ElementwiseReadOp::Add => backend.add_read(inputs[0].clone(), inputs[1].clone())?,
        ElementwiseReadOp::Subtract => backend.sub_read(inputs[0].clone(), inputs[1].clone())?,
        ElementwiseReadOp::Multiply => backend.mul_read(inputs[0].clone(), inputs[1].clone())?,
        ElementwiseReadOp::Negate => backend.neg_read(inputs[0].clone())?,
        ElementwiseReadOp::Conj => backend.conj_read(inputs[0].clone())?,
        ElementwiseReadOp::Divide => backend.div_read(inputs[0].clone(), inputs[1].clone())?,
    };
...
pub trait TensorElementwise: TensorStructural {
    ...
    ///
    /// Every backend must implement this hook with its explicit execution
    /// context, placement checks, and output-storage policy. The hook is the
    /// backend boundary for borrowed inputs and caller-owned output; it must not
    /// silently transfer data between devices.
    ///
    /// # Examples
    ///
    /// ```rust
    ...
    /// # Errors
    ///
    /// Returns [`crate::Error::Validation`] for invalid arity, metadata, or
    /// overlapping storage. Backend-specific placement, unsupported-operation,
    /// and execution errors are returned unchanged by the implementation.
