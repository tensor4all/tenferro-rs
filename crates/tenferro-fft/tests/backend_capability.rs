        ...
        impl TensorElementwise for $ty {
            ...
            unreachable_backend_methods! {
                ...
                fn sub_read(&mut self, lhs: TensorRead<'_>, rhs: TensorRead<'_>) -> tenferro_tensor::Result<Tensor> {
                    self.sub(tenferro_tensor::backend::read_owned_tensor("sub", lhs)?, tenferro_tensor::backend::read_owned_tensor("sub", rhs)?)
                }

                // Reproduce the previous read-half default: delegate an owned tensor and
                // reject a borrowed view.
                fn mul_read(&mut self, lhs: TensorRead<'_>, rhs: TensorRead<'_>) -> tenferro_tensor::Result<Tensor> {
                    self.mul(tenferro_tensor::backend::read_owned_tensor("mul", lhs)?, tenferro_tensor::backend::read_owned_tensor("mul", rhs)?)
                }
                ...
                fn select_read(&mut self, pred: TensorRead<'_>, on_true: TensorRead<'_>, on_false: TensorRead<'_>) -> tenferro_tensor::Result<Tensor> {
                    self.select(tenferro_tensor::backend::read_owned_tensor("select", pred)?, tenferro_tensor::backend::read_owned_tensor("select", on_true)?, tenferro_tensor::backend::read_owned_tensor("select", on_false)?)
                }
