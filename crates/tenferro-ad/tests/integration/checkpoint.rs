...
fn checkpoint_downstream_eval_uses_leaf() {
    ...
    let z = (&y + &one).unwrap();
    let value = get_f64_scalar(&z.run_with(&engine).unwrap());
...
fn grad_both_independently_checkpointed_add_wrt_rhs() {
    ...
    x2.checkpoint(&mut compiler, &executor).unwrap();
    y2.checkpoint(&mut compiler, &executor).unwrap();

    let z = (&x2 + &y2).unwrap();

    let grad_y = z.grad(&y).unwrap();
    let grad_y = grad_y;
    let grad_y_value = get_f64_scalar(&grad_y.run_with(&engine).unwrap());
    assert!(
...
fn grad_both_independently_checkpointed_add_wrt_lhs() {
    ...
    let grad_x = grad_x;
    let grad_x_value = get_f64_scalar(&grad_x.run_with(&engine).unwrap());
    assert!(
...
fn grad_both_independently_checkpointed_matches_fd() {
    ...
    let z = (&x2 + &y2).unwrap();
    let grad_y = z.grad(&y).unwrap();
    let ad_value = get_f64_scalar(&grad_y.run_with(&engine).unwrap());
    assert!(
        (ad_value - fd).abs() < TOL,
